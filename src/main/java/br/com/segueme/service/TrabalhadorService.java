...
public class TrabalhadorService implements Serializable {
    ...
    public Trabalhador salvar(Trabalhador trabalhador) {
        ...
        // Verificar se já existe um trabalhador para esta pessoa, equipe e encontro
        if (trabalhador.getEncontro() != null && trabalhador.getEncontro().getId() != null) {
            Optional<Trabalhador> trabalhadorExistente = trabalhadorRepository.findByPessoaEquipeEncontro(
                    trabalhador.getPessoa().getId(),
                    trabalhador.getEquipe().getId(),
                    trabalhador.getEncontro().getId());

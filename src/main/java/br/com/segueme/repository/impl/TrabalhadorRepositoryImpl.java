...
public class TrabalhadorRepositoryImpl implements TrabalhadorRepository {
	...
	public List<Trabalhador> findAllAtivos() {
		return entityManager.createQuery(
				"SELECT t FROM Trabalhador t " +
						...
						"LEFT JOIN FETCH en.encontro " +
						"LEFT JOIN FETCH t.contribuicoes " +
						"WHERE t.ativo = true", // Filtra apenas os ativos
				Trabalhador.class)
				.getResultList();
	}
	@Override
	public List<Trabalhador> findAllDistinct() {
		return entityManager.createQuery(
			"SELECT t FROM Trabalhador t " +
			"LEFT JOIN FETCH t.pessoa p " + // Carrega Pessoa
			"LEFT JOIN FETCH t.equipe e " + // Carrega Equipe
			"LEFT JOIN FETCH e.tipoEquipe " + // Carrega TipoEquipe
			"LEFT JOIN FETCH t.encontro " + // Carrega Encontro
			"LEFT JOIN FETCH t.encontrista en " + // Carrega Encontrista
			"LEFT JOIN FETCH en.pessoa " + // Carrega Pessoa do Encontrista
			"WHERE t.id IN (" +
